...
const cuiChineseRecord = BIBLIOGRAPHY_RECORDS.find(({ id }) => id === 'research-cuiXuesenQindingConstitution2015Chinese');
assert.ok(liberalConstitutionalismEntry.references.researchSourceIds.includes('cuiXuesenQindingConstitution2015Chinese'));
assert.ok(JSON.stringify(liberalConstitutionalismEntry.sections).includes('cuiXuesenQindingConstitution2015Chinese'));
assert.equal(cuiChineseRecord.evidenceRole, 'secondary');
assert.equal(cuiChineseRecord.publicationDate, '2015');
assert.equal(cuiChineseRecord.accessDate, '2026-09-19');
assert.deepEqual(cuiChineseRecord.languages, ['Chinese']);
assert.equal(cuiChineseRecord.review.confidence, 'medium');
assert.equal(cuiChineseRecord.publicationStatus, 'link-only');
assert.equal(cuiChineseRecord.directQuote, null);
assert.ok(cuiChineseRecord.relationships.profileEntries.includes('encyclopedia:liberal-constitutionalist'));
assert.ok(liberalConstitutionalismEntry.sections.find(({ id }) => id === 'introduction').blocks.some(({ text }) => text?.startsWith('A Chinese-language historiographical record')));
...
for (const [sourceId, role, date, languages] of [
  ...
  const expectedPortugueseProfiles = sourceId === 'portugalConstitution1976Official'
    ? ['encyclopedia:christian-democratic', 'geography:portugal-democratic-transition-constitutional-founding', 'geography:portuguese-democratic-consolidation-and-europeanization', 'geography:portuguese-2025-2026-electoral-transition']
...
assert.match(religiousDescription, /participant-level caution/);
...
for (const [sourceId, evidenceRole, publicationDate, confidence, languages] of [
  ...
  ['costaZangelmiSchiavoCebs2011Portuguese', 'secondary', '2011-06-29', 'medium', ['Portuguese']],
  ...
  assert.equal(record.publicationDate, publicationDate);
  assert.equal(record.review.confidence, confidence);
  assert.equal(record.accessDate, sourceId === 'freireBrazilLiberationNetwork2022' ? '2026-09-18' : sourceId === 'costaZangelmiSchiavoCebs2011Portuguese' ? '2026-09-19' : '2026-09-17');
  assert.deepEqual(record.languages, languages);
  assert.equal(record.publicationStatus, 'link-only');
  assert.equal(record.directQuote, null);
  assert.deepEqual(record.relationships.profileEntries, ['encyclopedia:religious-socialist']);
...
assert.ok(religiousSocialistEntry.researchGaps.some((gap) => gap.startsWith('This pass adds Costa, Zangelmi and Schiavo')));
