...
assert.deepEqual(dimensionIds, CORE_DIMENSION_IDS, 'the validation construct map must follow the public six-dimension order');
...
assert.equal(CANDIDATE_FACETS.length, 7, 'the validation plan should preserve the seven remaining candidate facets');
...
const m8Models = VALIDATION_MODEL_DEFINITIONS.filter(({ family }) => family === 'M8');
assert.equal(m5Models.length, 1, 'M5 must contain one stable baseline');
assert.equal(m6Models.length, 1, 'M6 must contain the current six-axis baseline');
assert.equal(m7Models.length, CANDIDATE_FACETS.length, 'M7 must test every remaining candidate facet separately');
