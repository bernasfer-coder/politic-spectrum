...
assert.equal(GEOGRAPHY_CASES.length, 147);
assert.equal(GEOGRAPHY_LABELS.length, 145);
...
assert.equal(filterGeographyCases({ continent: 'Oceania' }).length, 11);
...
assert.equal(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).length, 125, 'the atlas should expose all dated post-2000 implemented cases');
...
console.log('Geography tests passed: 147 dated cases, 145 unscored labels, citations, boundaries, filters and share URLs.');
