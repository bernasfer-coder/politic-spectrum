...
assert.equal(GEOGRAPHY_CASES.length, 148);
assert.equal(GEOGRAPHY_LABELS.length, 146);
...
assert.equal(filterGeographyCases({ continent: 'Oceania' }).length, 12);
...
assert.equal(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).length, 126, 'the atlas should expose all dated post-2000 implemented cases');
...
console.log('Geography tests passed: 148 dated cases, 146 unscored labels, citations, boundaries, filters and share URLs.');
