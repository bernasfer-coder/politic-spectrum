...
assert.equal(GEOGRAPHY_CASES.length, 159);
assert.equal(GEOGRAPHY_LABELS.length, 157);
const yemenCase = GEOGRAPHY_CASES.find(({ id }) => id === 'yemeni-unification-and-fragmented-state-conflict');
assert.ok(yemenCase, 'Yemen’s bounded political-history case is registered');
assert.equal(yemenCase.placeId, 'yemen');
assert.equal(yemenCase.startYear, 1990);
assert.equal(yemenCase.endYear, 2026);
assert.equal(yemenCase.reviewedAt, '2026-09-26');
assert.equal(GEOGRAPHY_PLACES.find(({ id }) => id === 'yemen')?.countryIds.includes('map-887'), true);
assert.ok(yemenCase.claim.includes('85,818 people as displaced'));
assert.ok(yemenCase.claim.includes('neither a complete census'));
assert.ok(yemenCase.limitation.includes('no national six-axis score'));
for (const sourceId of ['yemenUnionAgreement1990UNPeacemaker', 'yemenConstitution1991UNPeacemaker', 'yemenNationalDialogueOSESGY2014', 'yemenDreschModernHistory2000', 'yemenCarapicoCivilSociety1998', 'yemenDayRegionalismRebellion2012', 'yemenLacknerPovertyConflict2022', 'yemenBrandtTribesPolitics2017', 'yemenUNEnvoyBriefing2026', 'yemenAPMokhaOffensive2026', 'yemenIOMDisplacement2026', 'yemenAPSouthernConstitution2026']) {
  assert.ok(yemenCase.sourceIds.includes(sourceId), `Yemen source missing: ${sourceId}`);
  assert.equal(RIGHTS_RECORDS.researchSources[sourceId]?.reviewedAt, '2026-09-26', `Yemen rights/provenance review missing: ${sourceId}`);
}
...
console.log('Geography tests passed: 159 dated cases, 157 unscored labels, citations, boundaries, filters and share URLs.');
