...
assert.equal(GEOGRAPHY_CASES.length, 95);
assert.equal(GEOGRAPHY_LABELS.length, 93);
...
assert.equal(filterGeographyCases({ continent: 'Oceania' }).length, 5);
...
console.log('Geography tests passed: 95 dated cases, 93 unscored labels, citations, boundaries, filters and share URLs.');
