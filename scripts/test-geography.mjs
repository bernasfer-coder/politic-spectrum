...
const myanmarElectionCase = GEOGRAPHY_CASES.find(({ id }) => id === 'myanmar-2025-2026-election-and-presidential-transition');
assert.ok(myanmarElectionCase);
assert.equal(myanmarElectionCase.labelId, 'myanmar-2025-2026-election-and-presidential-transition');
assert.deepEqual([myanmarElectionCase.startYear, myanmarElectionCase.endYear, myanmarElectionCase.axisPositions], [2025, 2026, null]);
assert.ok(myanmarElectionCase.limitation.includes('no dedicated book-length study of this episode was located'));
assert.equal(GEOGRAPHY_LABELS.find(({ id }) => id === myanmarElectionCase.labelId).relatedEntries.length, 5);
