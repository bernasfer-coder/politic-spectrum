...
assert.equal(GEOGRAPHY_CASES.length, 220);
assert.equal(GEOGRAPHY_LABELS.length, 218);
const serbiaElection = GEOGRAPHY_CASES.find(({ id }) => id === 'serbia-early-parliamentary-election-2026-pre-election');
assert.equal(serbiaElection?.datePrecision, 'snapshot');
assert.equal(serbiaElection?.reviewedAt, '2026-09-26');
assert.ok(serbiaElection?.claim.includes('25 October'));
assert.ok(serbiaElection?.claim.includes('signed decision and Gazette itself were not retrieved'));
assert.ok(serbiaElection?.claim.includes('30 long-term observers'));
assert.ok(serbiaElection?.claim.includes('No ideological or six-axis classification is assigned'));
assert.ok(serbiaElection?.limitation.includes('No six-axis score'));
assert.ok(serbiaElection?.sourceIds.includes('serbiaBieberAuthoritarianism2020'));
assert.deepEqual(GEOGRAPHY_PLACES.find(({ id }) => id === serbiaElection?.placeId)?.countryIds, ['map-688']);
assert.equal(MAP_COUNTRIES_BY_ID['map-688']?.name, 'Serbia');
assert.ok(filterGeographyCases({ country: 'map-688' }).some(({ id }) => id === serbiaElection?.id));
...
assert.equal(israelElection2026?.confidence, 'medium');
assert.ok(israelElection2026?.claim.includes('unofficial translation'));
assert.ok(israelElection2026?.claim.includes('voted 18–5, with one abstention'));
assert.ok(israelElection2026?.claim.includes('requires Supreme Court approval'));
assert.ok(israelElection2026?.claim.includes('no Supreme Court disposition is claimed'));
assert.ok(israelElection2026?.limitation.includes('underlying CEC minutes or signed decisions were not located'));
assert.ok(israelElection2026?.limitation.includes('not a six-axis profile'));
assert.equal(GEOGRAPHY_PLACES.find(({ id }) => id === 'israel-26th-knesset-election-2026')?.countryIds[0], 'map-376');
assert.equal(MAP_COUNTRIES_BY_ID['map-376']?.name, 'Israel');
...
assert.ok(latviaElection2026?.limitation.includes('Snapshot frozen before polling'));
assert.ok(latviaElection2026?.limitation.includes('no ideological label or six-axis score is inferred'));
...
assert.deepEqual(GEOGRAPHY_PLACES.find(({ id }) => id === latviaElection2026?.placeId)?.countryIds, ['map-428']);
assert.equal(filterGeographyCases({ country: 'map-428' }).some(({ id }) => id === latviaElection2026?.id), true);
...
assert.equal(russianDuma2026?.reviewedAt, '2026-09-26');
assert.ok(russianDuma2026?.periodLabel.includes('scheduled first sitting'));
assert.ok(russianDuma2026?.claim.includes('349 seats'));
assert.ok(russianDuma2026?.claim.includes('355 United Russia seats'));
assert.ok(russianDuma2026?.claim.includes('30 September at 10:00'));
assert.ok(russianDuma2026?.claim.includes('publication record (0001202609250003)'));
assert.ok(russianDuma2026?.claim.includes('The sitting remained in the future'));
...
assert.ok(russianDuma2026?.limitation.includes('portal page timed out'));
assert.ok(russianDuma2026?.sourceIds.includes('russiaPresidentialDecreeFirstDumaSession2026'));
assert.ok(russianDuma2026?.sourceIds.includes('russiaInterfaxFirstDumaSession2026'));
assert.ok(russianDuma2026?.sourceIds.includes('russiaGarantDumaDecree2026'));
...
assert.equal(bosniaElection?.reviewedAt, '2026-09-26');
...
assert.ok(bosniaElection?.claim.includes('26 long-term observers deployed in the country'));
assert.ok(bosniaElection?.claim.includes('300 short-term observers will be requested'));
assert.ok(bosniaElection?.claim.includes('arrival scheduled for 30 September and deployment for 2 October'));
assert.ok(bosniaElection?.limitation.includes('future short-term schedule dates are not completed deployment'));
...
assert.ok(somaliaTransition?.sourceIds.includes('somaliaSenateConstitutionLibrarySeptember2026'));
assert.ok(somaliaTransition?.claim.includes('catalogue date of 5 September 2026'));
assert.ok(somaliaTransition?.claim.includes('cover is dated 4 March 2026'));
...
assert.ok(philippinesImpeachment2025?.claim.includes('No judicial disposition was located by this review cutoff'));
assert.ok(philippinesImpeachment2025?.claim.includes('Ernesto Francisco filed a separate certiorari petition'));
assert.ok(philippinesImpeachment2025?.claim.includes('planned to resume the trial on 28 September'));
...
assert.ok(philippinesImpeachment2025?.sourceIds.includes('philstarThresholdPetitionsAndResumption2026'));
...
assert.equal(moroccoElection2026?.reviewedAt, '2026-09-26');
assert.match(moroccoElection2026?.claim ?? '', /PAM 97 seats, RNI 66, Istiqlal 65 and PJD 54/);
assert.match(moroccoElection2026?.claim ?? '', /“definitive” constituency results.*not a Constitutional Court judgment/);
assert.match(moroccoElection2026?.claim ?? '', /portal still displayed its result\/validation workflow as in progress/);
...
assert.match(moroccoElection2026?.claim ?? '', /30 days after announcement of the poll result/);
assert.match(moroccoElection2026?.claim ?? '', /declared winners remain in office until the Court annuls their election/);
assert.match(moroccoElection2026?.limitation ?? '', /the 30-day appeal period has not been calculated/);
assert.match(moroccoElection2026?.limitation ?? '', /preliminary institutional observation/);
assert.match(moroccoElection2026?.limitation ?? '', /whether a petition was filed/);
...
assert.ok(moroccoElection2026?.sourceIds.includes('moroccoInteriorDefinitiveCommitteeResults2026'));
assert.ok(moroccoElection2026?.sourceIds.includes('moroccoConstitutionalCourtElectionDisputesLaw2026'));
assert.ok(moroccoElection2026?.sourceIds.includes('moroccoOrganicLawHouseRepresentatives2026'));
...
assert.equal(implementedPost2000.length, 192, 'the atlas should expose all dated post-2000 implemented cases');
...
assert.ok(implementedPost2000.includes('serbia-early-parliamentary-election-2026-pre-election'));
