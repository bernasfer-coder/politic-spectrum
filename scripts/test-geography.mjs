...
assert.equal(GEOGRAPHY_CASES.length, 31);
assert.equal(GEOGRAPHY_LABELS.length, 29);
...
assert.equal(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).length, 9, 'the atlas should expose all dated post-2000 implemented cases');
assert.deepEqual(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).map(({ id }) => id).sort(), ['antarctic-treaty-system', 'bhutanese-democratic-constitutional-transition', 'ghanaian-fourth-republic-constitutional-transition', 'jordanian-constitutional-monarchy', 'kuwait-constitutional-parliamentary-order', 'oman-sultani-constitutional-order', 'philippine-postauthoritarian-constitutionalism', 'saudi-basic-law-shura-order', 'tongan-constitutional-reform-1875-2010']);
...
console.log('Geography tests passed: 31 dated cases, 29 unscored labels, citations, boundaries, filters and share URLs.');
