...
const portugal2025to2026 = filterGeographyCases({ country: 'portugal' }).find(({ id }) => id === 'portuguese-2025-2026-electoral-transition');
assert.ok(portugal2025to2026, 'Portugal coverage should include the documented 2025–2026 electoral transition');
assert.equal(portugal2025to2026.relationship, 'influenced');
assert.equal(portugal2025to2026.axisPositions, null, 'a cross-party election and succession sequence should not receive a synthetic six-axis score');
assert.match(portugal2025to2026.limitation, /no event-specific book-length study was located/i);
...
assert.equal(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).length, 133, 'the atlas should expose all dated post-2000 implemented cases');
