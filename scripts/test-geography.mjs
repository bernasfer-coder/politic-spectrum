...
assert.equal(GEOGRAPHY_LABELS.length, 159);
...
assert.equal(filterGeographyCases({ country: 'mexico' })[2].id, 'mexican-judicial-reform-and-2025-federal-election');
...
const lebanon2022to2026 = filterGeographyCases({ country: 'map-422' }).find(({ id }) => id === 'lebanese-2022-2026-presidential-transition-and-sovereignty-contestation');
assert.ok(lebanon2022to2026);
assert.equal(lebanon2022to2026.startYear, 2022);
assert.equal(lebanon2022to2026.endYear, 2026);
assert.equal(lebanon2022to2026.datePrecision, 'approximate');
assert.equal(lebanon2022to2026.confidence, 'medium');
assert.match(lebanon2022to2026.limitation, /No located book-length scholarship evaluates the whole 2025–26 period/);
assert.ok(lebanon2022to2026.sourceIds.includes('lebanonParliamentTermExtension2026'));
assert.ok(lebanon2022to2026.sourceIds.includes('lebanonUNResponsePlanAddendum2026'));
assert.ok(GEOGRAPHY_LABELS.some(({ id }) => id === lebanon2022to2026.labelId));
assert.equal(filterGeographyCases({ country: 'map-706' })[0].id, 'somalia-2024-2026-constitutional-review-and-electoral-contestation');
const somalia2024to2026 = filterGeographyCases({ country: 'map-706' }).find(({ id }) => id === 'somalia-2024-2026-constitutional-review-and-electoral-contestation');
assert.ok(somalia2024to2026);
assert.equal(somalia2024to2026.startYear, 2024);
assert.equal(somalia2024to2026.endYear, 2026);
assert.equal(somalia2024to2026.datePrecision, 'approximate');
assert.equal(somalia2024to2026.confidence, 'medium');
assert.match(somalia2024to2026.claim, /Banadir\/Mogadishu district-council vote, not a nationwide/);
assert.match(somalia2024to2026.limitation, /No located book-length study treats this 2024–September 2026 sequence/);
assert.ok(somalia2024to2026.sourceIds.includes('somaliaConstitution2026Senate'));
assert.ok(somalia2024to2026.sourceIds.includes('bereketeabStateBuildingIdentityHorn2017'));
assert.ok(somalia2024to2026.sourceIds.includes('massoudShariaInshallahSomalia2021'));
for (const id of somalia2024to2026.sourceIds) assert.ok(context.sourceIds.has(id), `Somalia source is registered: ${id}`);
assert.ok(GEOGRAPHY_LABELS.find(({ id }) => id === somalia2024to2026.labelId)?.relatedEntries.some(({ id }) => id === 'liberal-constitutionalist'));
assert.ok(GEOGRAPHY_COUNTRIES.some(({ id, name }) => id === 'map-728' && name === 'South Sudan'));
assert.ok(GEOGRAPHY_COUNTRIES.some(({ id, name }) => id === 'map-348' && name === 'Hungary'));
assert.ok(GEOGRAPHY_PLACES.some(({ id, countryIds }) => id === 'hungary' && countryIds.includes('map-348')));
const hungary2026 = filterGeographyCases({ country: 'map-348' }).find(({ id }) => id === 'hungarian-2026-parliamentary-alternation-and-constitutional-transition');
assert.ok(hungary2026);
assert.equal(hungary2026.startYear, 2026);
assert.equal(hungary2026.endYear, 2026);
assert.equal(hungary2026.datePrecision, 'approximate');
assert.equal(hungary2026.confidence, 'medium');
assert.match(hungary2026.claim, /regime alteration/);
assert.match(hungary2026.limitation, /No book-length study of the 2026 election and transition was located/);
assert.ok(hungary2026.sourceIds.includes('hungaryFundamentalLawSeventeenthAmendment2026'));
assert.ok(hungary2026.sourceIds.includes('hungaryMikolaElectionImplications2026'));
for (const id of hungary2026.sourceIds) assert.ok(context.sourceIds.has(id), `Hungary source is registered: ${id}`);
assert.ok(GEOGRAPHY_LABELS.find(({ id }) => id === hungary2026.labelId)?.relatedEntries.some(({ id }) => id === 'liberal-constitutionalist'));
assert.ok(GEOGRAPHY_PLACES.some(({ id, countryIds }) => id === 'south-sudan' && countryIds.includes('map-728')));
assert.equal(filterGeographyCases({ country: 'map-728' })[0].id, 'south-sudan-2024-2026-r-arcss-transition-and-electoral-contestation');
const southSudan2024to2026 = filterGeographyCases({ country: 'map-728' })[0];
assert.equal(southSudan2024to2026.startYear, 2024);
assert.equal(southSudan2024to2026.endYear, 2026);
assert.equal(southSudan2024to2026.datePrecision, 'approximate');
assert.equal(southSudan2024to2026.confidence, 'medium');
assert.match(southSudan2024to2026.claim, /independent Republic of South Sudan, not Sudan/);
assert.match(southSudan2024to2026.limitation, /No book-length study covers this 2024–26 sequence/);
assert.ok(southSudan2024to2026.sourceIds.includes('southSudanNecVoterRegistration2026'));
assert.ok(southSudan2024to2026.sourceIds.includes('southSudanRoachFatefulStruggle2023'));
for (const id of southSudan2024to2026.sourceIds) assert.ok(context.sourceIds.has(id), `South Sudan source is registered: ${id}`);
assert.ok(GEOGRAPHY_LABELS.find(({ id }) => id === southSudan2024to2026.labelId)?.relatedEntries.some(({ id }) => id === 'liberal-constitutionalist'));
...
const haitiPostMoise = filterGeographyCases({ country: 'map-332' }).find(({ id }) => id === 'haitian-post-moise-transition-and-electoral-horizon');
assert.ok(haitiPostMoise);
assert.equal(haitiPostMoise.startYear, 2021);
assert.equal(haitiPostMoise.endYear, 2026);
assert.equal(haitiPostMoise.datePrecision, 'approximate');
assert.equal(haitiPostMoise.confidence, 'medium');
assert.match(haitiPostMoise.claim, /conditional CEP calendar/);
assert.match(haitiPostMoise.limitation, /not a score or profile of Haiti, Haitians/);
assert.ok(haitiPostMoise.sourceIds.includes('haitiPostMoiseCepSep21Status2026'));
assert.ok(haitiPostMoise.sourceIds.includes('haitiPostMoiseChathamPoll2026'));
assert.ok(GEOGRAPHY_LABELS.some(({ id }) => id === haitiPostMoise.labelId));
...
console.log('Geography tests passed: 161 dated cases, 159 unscored labels, citations, boundaries, filters and share URLs.');
