...
import { GEOGRAPHY_CASES, GEOGRAPHY_COUNTRIES, GEOGRAPHY_LABELS, GEOGRAPHY_RELATIONSHIPS } from '../src/content/geography.js';
import { validateGeography } from '../src/content/validate-geography.js';
import { MAP_COUNTRIES } from '../src/geography-map-model.js';
...
assert.ok(GEOGRAPHY_COUNTRIES.some(({ id, name }) => id === 'map-705' && name === 'Slovenia'));
assert.ok(MAP_COUNTRIES.some(({ id, name, path }) => id === 'map-705' && name === 'Slovenia' && path));
assert.equal(filterGeographyCases({ country: 'map-705' })[0].id, 'slovenian-2026-parliamentary-election-and-government-formation');
