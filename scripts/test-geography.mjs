...
assert.ok(bosniaElection?.limitation.includes('not a live update'));
...
assert.ok(southSudanTransition?.claim.includes('unresolved distinction'));
assert.ok(southSudanTransition?.claim.includes('does not warrant a single ideological label or six-axis placement'));
...
assert.ok(saoTomeElection?.claim.includes('scheduled for 27 September 2026'));
assert.ok(saoTomeElection?.claim.includes('had published no findings'));
...
assert.equal(implementedPost2000.length, 178, 'the atlas should expose all dated post-2000 implemented cases');
