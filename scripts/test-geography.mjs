...
assert.ok(filterGeographyCases({ country: 'japan' }).some(({ id }) => id === 'japanese-2025-2026-ldp-coalition-realignment-and-takaichi-government'));
...
assert.equal(filterGeographyCases({ country: 'indonesia' }).find(({ id }) => id === 'indonesian-prabowo-era-electoral-transition-and-civil-military-contestation')?.endYear, 2025);
...
assert.ok(filterGeographyCases({ country: 'map-410' }).some(({ id }) => id === 'south-korean-2025-impeachment-election-and-presidential-succession'));
assert.equal(filterGeographyCases({ country: 'map-410' }).find(({ id }) => id === 'south-korean-2025-impeachment-election-and-presidential-succession')?.periodLabel, 'April–June 2025 · impeachment disposition, vacancy election and presidential succession');
...
const peru2026 = filterGeographyCases({ country: 'map-604' }).find(({ id }) => id === 'peruvian-2026-general-election-bicameral-return-and-presidential-runoff');
assert.equal(peru2026?.periodLabel, 'April–July 2026 · general election, bicameral Congress and presidential runoff');
assert.ok(peru2026?.sourceIds.includes('peruJneResolution1610RunoffReview2026'));
assert.ok(peru2026?.sourceIds.includes('peruEUElectionObservationFinal2026'));
