...
const liberiaWecc = GEOGRAPHY_CASES.find(({ id }) => id === 'liberia-war-economic-crimes-court-establishment-process-2009-2026');
assert.equal(liberiaWecc?.relationship, 'influenced');
assert.equal(liberiaWecc?.startYear, 2009);
assert.equal(liberiaWecc?.endYear, 2026);
assert.ok(liberiaWecc?.claim.includes('not an enacted or functioning WECC'));
assert.ok(liberiaWecc?.limitation.includes('latest located status'));
assert.ok(liberiaWecc?.sourceIds.includes('liberiaTrcFindingsVolumeOne2009'));
assert.ok(liberiaWecc?.sourceIds.includes('liberiaTrcConsolidatedReportVolumeTwo2009'));
assert.ok(liberiaWecc?.limitation.includes('Volume II is listed only as an official research path'));
...
assert.equal(greeceReferendum2015?.endYear, 2015);
assert.equal(greeceReferendum2015?.confidence, 'medium');
assert.ok(greeceReferendum2015?.claim.includes('not a direct vote to leave or remain in the euro'));
assert.ok(greeceReferendum2015?.claim.includes('3,558,864 (61.31%) “No”'));
assert.ok(greeceReferendum2015?.claim.includes('one common reason for voting'));
assert.ok(greeceReferendum2015?.limitation.includes('two fully read, independent monographic treatments'));
assert.ok(greeceReferendum2015?.sourceIds.includes('greeceReferendumDecree382015'));
...
const romaniaElection = GEOGRAPHY_CASES.find(({ id }) => id === 'romania-2024-presidential-annulment-and-2025-rerun');
assert.equal(romaniaElection?.relationship, 'implemented');
assert.equal(romaniaElection?.startYear, 2024);
assert.equal(romaniaElection?.endYear, 2025);
assert.equal(romaniaElection?.confidence, 'medium');
assert.ok(romaniaElection?.claim.includes('annulled the entire presidential-election process'));
assert.ok(romaniaElection?.claim.includes('expressly did not assess the facts'));
assert.ok(romaniaElection?.claim.includes('53.6 per cent'));
assert.ok(romaniaElection?.limitation.includes('Móré’s Romanian-language article'));
assert.ok(romaniaElection?.limitation.includes('do not substantiate event-level claims'));
assert.ok(romaniaElection?.sourceIds.includes('romaniaConstitutionalCourtDecision322024'));
assert.ok(romaniaElection?.sourceIds.includes('romaniaVeniceCommissionAnnulmentReport2025'));
assert.ok(romaniaElection?.sourceIds.includes('romaniaOdihrRepeatPresidentialFinal2025'));
assert.ok(romaniaElection?.sourceIds.includes('romaniaConstitutionalCourtDecision422025'));
assert.ok(romaniaElection?.sourceIds.includes('romaniaLorenzMarisPoliticsSociety2026'));
assert.ok(romaniaElection?.sourceIds.includes('romaniaRametHassenstabPolitics1989Second2019'));
assert.equal(GEOGRAPHY_COUNTRIES.find(({ id }) => id === 'map-642')?.name, 'Romania');
assert.equal(MAP_COUNTRIES_BY_ID['map-642']?.name, 'Romania');
...
const moldovaTransition = GEOGRAPHY_CASES.find(({ id }) => id === 'moldova-constitutional-eu-accession-and-electoral-transition-2024-26');
assert.equal(moldovaTransition?.startYear, 2024);
assert.equal(moldovaTransition?.endYear, 2026);
assert.equal(moldovaTransition?.confidence, 'medium');
assert.ok(moldovaTransition?.claim.includes('749,719 valid YES votes and 739,155 NO votes'));
assert.ok(moldovaTransition?.claim.includes('negotiations remained in progress, not accession'));
assert.ok(moldovaTransition?.limitation.includes('not evidence of a uniform public preference'));
assert.ok(moldovaTransition?.limitation.includes('Event-specific book-length scholarship'));
assert.ok(moldovaTransition?.sourceIds.includes('moldovaOdihrParliamentaryElection2025'));
assert.ok(moldovaTransition?.sourceIds.includes('moldovaWayPluralismDefault2015'));
assert.equal(filterGeographyCases({ country: 'map-498' })[0]?.id, moldovaTransition?.id);
assert.equal(filterGeographyCases({ place: 'moldova-constitutional-eu-accession-and-electoral-transition-2024-26' })[0]?.id, moldovaTransition?.id);
assert.ok(!('scores' in GEOGRAPHY_LABELS.find(({ id }) => id === moldovaTransition?.labelId)));
assert.equal(GEOGRAPHY_COUNTRIES.find(({ id }) => id === 'map-498')?.name, 'Moldova');
assert.equal(MAP_COUNTRIES_BY_ID['map-498']?.name, 'Moldova');
