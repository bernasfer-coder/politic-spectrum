...
const ukraineCase = GEOGRAPHY_CASES.find(({ id }) => id === 'ukrainian-wartime-constitutional-order-and-accountability-2022-2026');
assert.ok(ukraineCase, 'Ukraine’s 2022–2026 wartime constitutional and accountability history is one bounded case');
assert.equal(ukraineCase.placeId, 'ukraine');
assert.equal(ukraineCase.startYear, 2022);
assert.equal(ukraineCase.endYear, 2026);
assert.equal(ukraineCase.reviewedAt, '2026-09-24');
assert.equal(ukraineCase.axisPositions, null, 'the Ukraine wartime case has no unsupported numeric axis scores');
assert.ok(ukraineCase.claim.includes('approved it by 300 votes'));
assert.ok(ukraineCase.claim.includes('reported through 31 October 2026'));
assert.ok(ukraineCase.claim.includes('These are dated legal facts and attributed scholarly or institutional interpretations—not scores'));
assert.ok(ukraineCase.limitation.includes('not a current claim beyond that evidence horizon'));
assert.ok(ukraineCase.limitation.includes('remain open research tasks'));
assert.ok(ukraineCase.sourceIds.includes('ukraineKurasStateTransformation2025'));
assert.ok(ukraineCase.sourceIds.includes('ukraineOHCHRReport2026'));
assert.ok(ukraineCase.sourceIds.every((sourceId) => RESEARCH_SOURCES.some(({ id }) => id === sourceId)), 'all Ukraine case sources resolve');
assert.equal(GEOGRAPHY_PLACES.find(({ id }) => id === 'ukraine')?.countryIds[0], 'map-804');
assert.equal(filterGeographyCases({ country: 'map-804' })[0].id, ukraineCase.id);
const sudanWar = GEOGRAPHY_CASES.find(({ id }) => id === 'sudan-war-and-fragmented-authority-2023-2025');
assert.ok(sudanWar, 'the 2023–2025 Sudan war and fragmented-authority episode is represented as one bounded case');
assert.equal(sudanWar.placeId, 'sudan');
assert.equal(sudanWar.startYear, 2023);
assert.equal(sudanWar.endYear, 2025);
assert.equal(sudanWar.axisPositions, null, 'the Sudan war case has no unsupported numerical axis scores');
assert.ok(sudanWar.claim.includes('not a criminal court’s judgment'));
assert.ok(sudanWar.claim.includes('Elamin’s December 2025 research article interprets'));
assert.ok(sudanWar.limitation.includes('not a current 2026 status claim'));
assert.ok(sudanWar.sourceIds.includes('sudanHumanRightsReport2026'));
assert.ok(sudanWar.sourceIds.includes('berridgeUnfinishedDemocracy2022'), 'prewar book-length scholarship is distinguished from a book about the war');
...
assert.ok(filterGeographyCases({ country: 'sudan' }).some(({ id }) => id === 'sudan-war-and-fragmented-authority-2023-2025'));
