...
const ethiopia2026 = filterGeographyCases({ country: 'ethiopia' }).find(({ id }) => id === 'ethiopian-2026-general-election-and-electoral-contestation');
assert.ok(ethiopia2026, 'the bounded 2026 Ethiopia election case should be discoverable by country');
assert.equal(ethiopia2026.startYear, 2026);
assert.equal(ethiopia2026.endYear, 2026);
assert.equal(ethiopia2026.sourceIds.length, 8);
assert.match(ethiopia2026.limitation, /CECOE lists a final report.*contents were not accessed/);
assert.match(ethiopia2026.claim, /without supporting a numeric placement/);
...
const philippineImpeachmentCase = filterGeographyCases({ country: 'philippines' }).find(({ id }) => id === 'philippine-vice-presidential-impeachment-and-senate-trial-2026');
assert.ok(philippineImpeachmentCase, 'the 2026 impeachment and trial should be a separate, discoverable case');
assert.equal(philippineImpeachmentCase.relationship, 'implemented');
assert.equal(philippineImpeachmentCase.startYear, 2026);
assert.equal(philippineImpeachmentCase.endYear, 2026);
assert.equal(philippineImpeachmentCase.sourceIds.length, 19);
assert.match(philippineImpeachmentCase.claim, /did not decide the merits of those challenges or Duterte’s guilt/);
assert.match(philippineImpeachmentCase.claim, /would not itself set a new numerical threshold/);
assert.match(philippineImpeachmentCase.limitation, /no book-length study of this live 2026 episode was identified/);
assert.ok(!('profile' in GEOGRAPHY_LABELS.find(({ id }) => id === philippineImpeachmentCase.labelId)));
...
assert.equal(filterGeographyCases({ continent: 'Oceania' }).length, 15);
...
const vanuatuReformCase = GEOGRAPHY_CASES.find(({ id }) => id === 'vanuatuan-party-reform-and-parliamentary-accountability-2023-2025');
assert.ok(vanuatuReformCase);
assert.equal(vanuatuReformCase.startYear, 2023);
assert.equal(vanuatuReformCase.endYear, 2025);
assert.equal(vanuatuReformCase.relationship, 'implemented');
assert.match(vanuatuReformCase.claim, /does not assert the motion’s eventual outcome/);
assert.match(vanuatuReformCase.limitation, /No 2026 status is asserted/);
assert.ok(!('profile' in GEOGRAPHY_LABELS.find(({ id }) => id === vanuatuReformCase.labelId)));
assert.equal(filterGeographyCases({ country: 'map-548' }).find(({ id }) => id === vanuatuReformCase.id)?.id, vanuatuReformCase.id);
const tongaElectionCase = GEOGRAPHY_CASES.find(({ id }) => id === 'tongan-election-and-post-election-accountability-2025-2026');
assert.ok(tongaElectionCase);
assert.equal(tongaElectionCase.startYear, 2025);
assert.equal(tongaElectionCase.endYear, 2026);
assert.equal(tongaElectionCase.reviewedAt, '2026-09-24');
assert.match(tongaElectionCase.claim, /not a direct popular election of the Prime Minister/);
assert.match(tongaElectionCase.claim, /no result is asserted/);
assert.match(tongaElectionCase.limitation, /court index does not provide the judgments’ reasoning/);
assert.match(tongaElectionCase.limitation, /no event-specific book-length study/);
assert.ok(!('profile' in GEOGRAPHY_LABELS.find(({ id }) => id === tongaElectionCase.labelId)));
assert.equal(filterGeographyCases({ country: 'tonga' }).find(({ id }) => id === tongaElectionCase.id)?.id, tongaElectionCase.id);
const syriaTransitionCase = GEOGRAPHY_CASES.find(({ id }) => id === 'syrian-post-assad-transition-and-state-reconstruction-2024-2026');
assert.ok(syriaTransitionCase);
assert.equal(syriaTransitionCase.startYear, 2024);
assert.equal(syriaTransitionCase.endYear, 2026);
assert.equal(syriaTransitionCase.reviewedAt, '2026-09-24');
assert.equal(syriaTransitionCase.sourceIds.length, 9);
assert.match(syriaTransitionCase.claim, /automated translation/);
assert.match(syriaTransitionCase.claim, /no evidence of a governmental policy or plan/);
assert.match(syriaTransitionCase.limitation, /not counted as a second scholarly perspective/);
assert.match(syriaTransitionCase.limitation, /not a score for Syria, Syrians/);
assert.ok(!('profile' in GEOGRAPHY_LABELS.find(({ id }) => id === syriaTransitionCase.labelId)));
assert.equal(filterGeographyCases({ country: 'syria' }).find(({ id }) => id === syriaTransitionCase.id)?.id, syriaTransitionCase.id);
assert.equal(context.bibliography.filter(({ citationIds }) => citationIds.researchSourceIds.includes('syriaPomepsStudies57')).length, 1);
const coteDivoireElectionCase = GEOGRAPHY_CASES.find(({ id }) => id === 'cote-divoire-2025-election-and-post-election-institutions');
assert.ok(coteDivoireElectionCase);
assert.equal(coteDivoireElectionCase.startYear, 2025);
assert.equal(coteDivoireElectionCase.endYear, 2026);
assert.equal(coteDivoireElectionCase.reviewedAt, '2026-09-24');
assert.equal(coteDivoireElectionCase.sourceIds.length, 9);
assert.match(coteDivoireElectionCase.claim, /54,977 valid sponsorships against 75,003 required/);
assert.match(coteDivoireElectionCase.claim, /different attributions and are not reconciled/);
assert.match(coteDivoireElectionCase.claim, /not an independent measure of competition/);
assert.match(coteDivoireElectionCase.limitation, /not an assessment of all Ivorian politics/);
assert.match(coteDivoireElectionCase.limitation, /publisher metadata, not full-text consultation/);
assert.ok(!('profile' in GEOGRAPHY_LABELS.find(({ id }) => id === coteDivoireElectionCase.labelId)));
...
const kazakhstanTransition = GEOGRAPHY_CASES.find(({ id }) => id === 'kazakhstani-2026-constitutional-reset-and-kurultai-transition');
assert.ok(kazakhstanTransition);
assert.equal(kazakhstanTransition.startYear, 2026);
assert.equal(kazakhstanTransition.endYear, 2026);
assert.equal(kazakhstanTransition.datePrecision, 'year');
assert.equal(kazakhstanTransition.relationship, 'implemented');
assert.equal(kazakhstanTransition.reviewedAt, '2026-09-24');
assert.equal(kazakhstanTransition.sourceIds.length, 11);
assert.match(kazakhstanTransition.claim, /73\.12%/);
assert.match(kazakhstanTransition.claim, /87\.15%/);
assert.match(kazakhstanTransition.claim, /not an independent audit/);
assert.match(kazakhstanTransition.claim, /labels itself unofficial/);
assert.match(kazakhstanTransition.claim, /preliminary statement/);
assert.match(kazakhstanTransition.limitation, /a score for Kazakhstan or Kazakhstanis/);
assert.match(kazakhstanTransition.limitation, /publisher descriptions and bibliographic records/);
assert.ok(!('profile' in GEOGRAPHY_LABELS.find(({ id }) => id === kazakhstanTransition.labelId)));
assert.ok(filterGeographyCases({ country: 'map-398' }).some(({ id }) => id === kazakhstanTransition.id));
