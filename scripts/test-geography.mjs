...
assert.equal(GEOGRAPHY_CASES.length, 32);
assert.equal(GEOGRAPHY_LABELS.length, 30);
...
assert.equal(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).length, 10, 'the atlas should expose all dated post-2000 implemented cases');
assert.deepEqual(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).map(({ id }) => id).sort(), ['antarctic-treaty-system', 'bhutanese-democratic-constitutional-transition', 'ghanaian-fourth-republic-constitutional-transition', 'indonesian-reformasi-constitutionalism', 'jordanian-constitutional-monarchy', 'kuwait-constitutional-parliamentary-order', 'oman-sultani-constitutional-order', 'philippine-postauthoritarian-constitutionalism', 'saudi-basic-law-shura-order', 'tongan-constitutional-reform-1875-2010']);
...
console.log('Geography tests passed: 32 dated cases, 30 unscored labels, citations, boundaries, filters and share URLs.');
