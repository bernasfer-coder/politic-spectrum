...
assert.equal(GEOGRAPHY_CASES.length, 113);
assert.equal(GEOGRAPHY_LABELS.length, 111);
assert.equal(filterGeographyCases({ country: 'singapore' }).length, 1);
assert.equal(filterGeographyCases({ country: 'singapore' })[0].id, 'singapore-2025-general-election');
const samoa2025Case = GEOGRAPHY_CASES.find(({ id }) => id === 'samoa-2025-snap-election-and-parliamentary-transition');
assert.equal(samoa2025Case.labelId, 'samoa-2025-electoral-and-parliamentary-transition');
assert.equal(samoa2025Case.datePrecision, 'year');
assert.equal(filterGeographyCases({ country: 'map-882' }).length, 2, 'Samoa has two separately bounded dated cases');
assert.ok(!('profile' in GEOGRAPHY_LABELS.find(({ id }) => id === samoa2025Case.labelId)), 'the 2025 election label remains unscored');
...
assert.equal(filterGeographyCases({ continent: 'Oceania' }).length, 8);
...
console.log('Geography tests passed: 113 dated cases, 111 unscored labels, citations, boundaries, filters and share URLs.');
