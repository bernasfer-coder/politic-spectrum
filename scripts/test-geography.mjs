...
const haitiTransition = GEOGRAPHY_CASES.find(({ id }) => id === 'haitian-post-assassination-transition-and-electoral-reconstruction-2021-2026');
assert.equal(haitiTransition?.startYear, 2021);
assert.equal(haitiTransition?.endYear, 2026);
assert.equal(haitiTransition?.datePrecision, 'approximate');
assert.match(haitiTransition?.periodLabel ?? '', /7 July 2021–21 September 2026/);
assert.match(haitiTransition?.claim ?? '', /candidate filing still in progress/);
assert.match(haitiTransition?.claim ?? '', /not the complete book/);
assert.match(haitiTransition?.claim ?? '', /no numeric axis score/);
assert.match(haitiTransition?.limitation ?? '', /2017 through 6 July 2021 remains uncovered/);
assert.match(haitiTransition?.limitation ?? '', /not a completed vote/);
assert.equal(haitiTransition?.sourceIds.length, 14);
assert.equal(filterGeographyCases({ country: 'map-332' }).length, 2);
assert.ok(filterGeographyCases({ country: 'map-332' }).some(({ id }) => id === 'haitian-post-assassination-transition-and-electoral-reconstruction-2021-2026'));
const usExecutive = GEOGRAPHY_CASES.find(({ id }) => id === 'united-states-second-trump-executive-power-and-contestation-2025-2026');
assert.equal(usExecutive?.startYear, 2025);
assert.equal(usExecutive?.endYear, 2026);
assert.equal(usExecutive?.datePrecision, 'approximate');
assert.match(usExecutive?.periodLabel ?? '', /20 January 2025–30 June 2026/);
assert.match(usExecutive?.claim ?? '', /expressly did not decide the order’s merits/);
assert.match(usExecutive?.evidenceKind ?? '', /opening-chapter excerpt for Edelson and contents\/foreword sample for Zick/);
assert.match(usExecutive?.claim ?? '', /postwar expansion, especially after 9\/11/);
assert.match(usExecutive?.claim ?? '', /do not establish the book’s conclusions/);
assert.match(usExecutive?.limitation ?? '', /neither complete book was read/);
assert.match(usExecutive?.limitation ?? '', /case remains incomplete at the book level/);
assert.equal(usExecutive?.sourceIds.length, 14);
assert.equal(filterGeographyCases({ country: 'map-840' }).length, 2);
assert.match(GEOGRAPHY_PLACES.find(({ id }) => id === 'united-states')?.note ?? '', /separate from a narrower second-administration case/);
const sudanTransition = GEOGRAPHY_CASES.find(({ id }) => id === 'sudanese-post-coup-transition-crisis-2021-2023');
assert.equal(sudanTransition?.startYear, 2021);
assert.equal(sudanTransition?.endYear, 2023);
assert.equal(sudanTransition?.datePrecision, 'approximate');
assert.match(sudanTransition?.periodLabel ?? '', /25 October 2021–14 April 2023/);
assert.match(sudanTransition?.claim ?? '', /rather than a settled causal account/);
assert.match(sudanTransition?.claim ?? '', /do not establish a numeric six-axis placement/);
assert.match(sudanTransition?.limitation ?? '', /full-text reading/);
assert.equal(sudanTransition?.sourceIds.length, 7);
assert.match(GEOGRAPHY_PLACES.find(({ id }) => id === 'sudan')?.note ?? '', /25 October 2021–14 April 2023/);
...
assert.match(sudanWar?.limitation ?? '', /Arabic-language scholarship, local archives and community-led testimony/);
...
const bangladeshTransition = GEOGRAPHY_CASES.find(({ id }) => id === 'bangladeshi-post-uprising-interim-and-electoral-transition');
assert.equal(bangladeshTransition?.startYear, 2024);
assert.equal(bangladeshTransition?.endYear, 2026);
assert.equal(bangladeshTransition?.datePrecision, 'approximate');
assert.match(bangladeshTransition?.periodLabel ?? '', /5 August 2024–17 February 2026/);
assert.match(bangladeshTransition?.claim ?? '', /only at the publisher’s accessible abstract\/metadata level, not as full-text reading/);
assert.match(bangladeshTransition?.claim ?? '', /No numeric six-axis placement is warranted/);
assert.match(bangladeshTransition?.limitation ?? '', /Bangla-language scholarship, domestic-observer and party sources/);
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshMonsoonUprisingForthcoming2026'));
assert.match(GEOGRAPHY_PLACES.find(({ id }) => id === 'bangladesh')?.note ?? '', /5 August 2024–17 February 2026/);
const syriaTransition = GEOGRAPHY_CASES.find(({ id }) => id === 'syrian-post-assad-central-state-transition-2024-2026');
assert.equal(syriaTransition?.startYear, 2024);
assert.equal(syriaTransition?.endYear, 2026);
assert.equal(syriaTransition?.datePrecision, 'approximate');
assert.match(syriaTransition?.claim ?? '', /inaugural sitting took place on 12 July 2026, not in June/);
assert.match(syriaTransition?.claim ?? '', /no numeric six-axis placement/);
assert.match(syriaTransition?.limitation ?? '', /one fully consulted book-length scholarly collection rather than two independently read monographs/);
assert.ok(syriaTransition?.sourceIds.includes('syriaPostAssadDeclaration2025'));
assert.ok(syriaTransition?.sourceIds.includes('syriaPostAssadAPAssemblySession2026'));
assert.match(GEOGRAPHY_PLACES.find(({ id }) => id === 'syria')?.note ?? '', /post-Assad central-state transition case reviewed through 21 September 2026/);
assert.ok(filterGeographyCases({ country: 'syria' }).some(({ id }) => id === 'syrian-post-assad-central-state-transition-2024-2026'));
...
assert.deepEqual(filterGeographyCases({ country: 'sudan' }).map(({ id }) => id), ['sudanese-islamist-military-and-revolutionary-transition', 'sudanese-post-coup-transition-crisis-2021-2023', 'sudanese-2023-war-and-contested-transition']);
