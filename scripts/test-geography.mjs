...
assert.ok(southAfricanGnuCase, 'the post-2024 GNU episode has its own bounded South Africa case');
...
assert.ok(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).some(({ id }) => id === southAfricanGnuCase.id));
...
const mozambiquePost2024 = filterGeographyCases({ country: 'mozambique' }).find(({ id }) => id === 'mozambican-post-2024-election-crisis-and-national-dialogue');
assert.ok(mozambiquePost2024);
assert.equal(mozambiquePost2024.startYear, 2024);
assert.equal(mozambiquePost2024.endYear, 2026);
assert.equal(mozambiquePost2024.confidence, 'medium');
assert.ok(mozambiquePost2024.claim.includes('casualty totals are omitted'));
assert.ok(mozambiquePost2024.limitation.includes('No book-length study focused on the 2024–2026 sequence'));
const mozambiquePost2024Label = GEOGRAPHY_LABELS.find(({ id }) => id === mozambiquePost2024.labelId);
assert.deepEqual(mozambiquePost2024Label.relatedEntries.map(({ id }) => id), ['liberal-constitutionalist', 'civic-nationalist', 'anti-colonial-liberation']);
assert.ok(mozambiquePost2024.sourceIds.includes('mozambiqueInclusiveDialogueLaw2025'));
assert.ok(mozambiquePost2024.sourceIds.includes('mozambiqueBarrosoDialogueBrief2026'));
...
const zambia2026 = filterGeographyCases({ country: 'map-894' }).find(({ id }) => id === 'zambian-2026-general-election-and-post-election-contestation');
assert.ok(zambia2026);
assert.equal(zambia2026.startYear, 2025);
assert.equal(zambia2026.endYear, 2026);
assert.equal(zambia2026.confidence, 'medium');
assert.ok(zambia2026.sourceIds.includes('zambiaCCMGPRVTResults2026'));
assert.ok(zambia2026.limitation.includes('book-length study focused on the 2025–2026 episode was located'));
