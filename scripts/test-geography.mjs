...
const nepaliUprising = filterGeographyCases({ country: 'nepal' }).find(({ id }) => id === 'nepali-gen-z-uprising-and-electoral-accountability-transition');
assert.ok(nepaliUprising, 'Nepal should include its bounded 2025–2026 protest and electoral-accountability case');
assert.equal(nepaliUprising.relationship, 'influenced');
assert.equal(nepaliUprising.startYear, 2025);
assert.equal(nepaliUprising.endYear, 2026);
assert.equal(nepaliUprising.axisPositions, null, 'the episode has no unsupported numerical six-axis profile');
assert.ok(nepaliUprising.limitation.includes('not a current-country assessment'));
const lebaneseWartimeTransition = filterGeographyCases({ country: 'map-422' }).find(({ id }) => id === 'lebanese-post-2022-wartime-transition-and-parliamentary-extension');
assert.ok(lebaneseWartimeTransition, 'Lebanon should include its bounded post-2022 wartime and parliamentary transition');
assert.equal(lebaneseWartimeTransition.relationship, 'influenced');
assert.equal(lebaneseWartimeTransition.startYear, 2022);
assert.equal(lebaneseWartimeTransition.endYear, 2026);
assert.equal(lebaneseWartimeTransition.axisPositions, null, 'the episode has no unsupported numerical six-axis profile');
assert.ok(lebaneseWartimeTransition.claim.includes('18 September 2026'));
assert.ok(lebaneseWartimeTransition.claim.includes('US$11 billion'));
assert.ok(lebaneseWartimeTransition.limitation.includes('No book-length scholarly study'));
assert.equal(filterGeographyCases({ country: 'map-422' }).length, 2, 'both Lebanon cases should remain independently bounded');
...
const boliviaTransition = GEOGRAPHY_CASES.find(({ id }) => id === 'bolivian-mas-fracture-and-2025-electoral-alternation');
assert.ok(boliviaTransition, 'Bolivia should have a bounded 2020–2025 electoral-transition case');
assert.equal(boliviaTransition.axisPositions, null, 'unsupported numeric six-axis coordinates must remain unassigned');
assert.ok(boliviaTransition.claim.includes('municipality-level aggregate'), 'the election analysis should state its ecological-inference limit');
...
const guyanaTransition = GEOGRAPHY_CASES.find(({ id }) => id === 'guyanese-post-2020-oil-boom-and-electoral-transition');
assert.ok(guyanaTransition, 'Guyana should have a bounded post-2020 oil-economy and electoral-transition case');
assert.equal(guyanaTransition.axisPositions, null, 'unsupported numeric six-axis coordinates must remain unassigned');
assert.ok(guyanaTransition.limitation.includes('5 August 2026'), 'the Guyana case should state its dated endpoint');
assert.ok(guyanaTransition.claim.includes('not evidence of individual voter motives'), 'election evidence must not be used for ecological inference');
...
const coteElectoralTransition = GEOGRAPHY_CASES.find(({ id }) => id === 'cote-ivoire-post-2020-electoral-and-constitutional-contestation');
assert.ok(coteElectoralTransition, 'Côte d’Ivoire should have a bounded post-2020 electoral-contestation case');
assert.equal(coteElectoralTransition.endYear, 2026);
assert.equal(coteElectoralTransition.axisPositions, null, 'unsupported numeric six-axis coordinates must remain unassigned');
assert.ok(coteElectoralTransition.limitation.includes('13 March 2026'), 'the Côte d’Ivoire case should give its verified endpoint');
assert.ok(coteElectoralTransition.claim.includes('prospective assessment'), 'observer scope and sequence must be distinguished');
assert.ok(filterGeographyCases({ country: 'cote-divoire' }).some(({ id }) => id === 'cote-ivoire-post-2020-electoral-and-constitutional-contestation'));
