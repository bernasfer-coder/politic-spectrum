...
const bangladeshTransition = GEOGRAPHY_CASES.find(({ id }) => id === 'bangladeshi-post-uprising-election-and-july-charter-contestation');
assert.equal(bangladeshTransition?.placeId, 'bangladesh');
assert.equal(bangladeshTransition?.startYear, 2024);
assert.equal(bangladeshTransition?.endYear, 2026);
assert.equal(bangladeshTransition?.relationship, 'experimented');
assert.equal(bangladeshTransition?.reviewedAt, '2026-09-23');
assert.match(bangladeshTransition?.claim ?? '', /47,225,980 Yes/);
assert.match(bangladeshTransition?.claim ?? '', /not proof of informed consent/);
assert.match(bangladeshTransition?.claim ?? '', /six-axis ideological interpretation is not justified/);
assert.match(bangladeshTransition?.limitation ?? '', /No published book-length study spanning the full 2024–2026 transition was identified/);
assert.match(bangladeshTransition?.limitation ?? '', /remain research gaps/);
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshJulyCharterImplementationOrder2025'));
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshCommonwealthObserverReport2026'));
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshCharterImplementationDebateBdnews242026'));
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshNationsAspirationsJulyUprising2026'));
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshJulyUprisingBibliographicReviewRahman2026'));
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshHossainBergmanMonsoonUprisingForthcoming2026'));
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshCriticalMediaPerspectivesUprising2026'));
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshShahabuddinInternationalLawBookForthcoming2026'));
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshJulyResolveFirstPersonStories2026'));
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshJulyUprisingPrisonLifeBookCatalog2025'));
assert.ok(bangladeshTransition?.sourceIds.includes('bangladeshElectionCommissionAwamiLeagueSuspension2025'));
const bangladeshProfile = GEOGRAPHY_LABELS.find(({ id }) => id === bangladeshTransition?.id);
assert.ok(bangladeshProfile?.sourceIds.includes('bangladeshCriticalMediaPerspectivesUprising2026'));
assert.ok(bangladeshProfile?.sourceIds.includes('bangladeshJulyResolveFirstPersonStories2026'));
assert.match(bangladeshTransition?.limitation ?? '', /not a comprehensive publisher\/library survey/);
assert.match(bangladeshTransition?.claim ?? '', /publisher record for a 368-page edited book/);
assert.match(bangladeshTransition?.claim ?? '', /curated first-person accounts/);
assert.match(bangladeshTransition?.limitation ?? '', /catalog entry are bibliographic records only; none was read/);
assert.match(bangladeshTransition?.limitation ?? '', /does not mean no other books about the 2024 uprising exist/);
assert.equal(filterGeographyCases({ country: 'bangladesh' }).find(({ id }) => id === bangladeshTransition?.id)?.endYear, 2026);
const syriaTransition = GEOGRAPHY_CASES.find(({ id }) => id === 'syrian-post-assad-constitutional-transition-2024-2026');
assert.equal(syriaTransition?.placeId, 'syria');
assert.equal(syriaTransition?.reviewedAt, '2026-09-23');
assert.match(syriaTransition?.limitation ?? '', /not a profile or ideology score/);
assert.match(syriaTransition?.limitation ?? '', /remain research gaps/);
...
assert.equal(filterGeographyCases({ country: 'map-124' }).find(({ id }) => id === 'canadian-carney-economic-sovereignty-and-parliamentary-realignment')?.endYear, 2026);
...
const nepalGenZTransition = filterGeographyCases({ country: 'nepal' }).find(({ id }) => id === 'nepali-gen-z-mobilization-and-2026-electoral-transition');
assert.equal(nepalGenZTransition?.startYear, 2025);
assert.equal(nepalGenZTransition?.endYear, 2026);
assert.equal(nepalGenZTransition?.relationship, 'influenced');
assert.ok(nepalGenZTransition?.sourceIds.includes('nepalNhcrGenZRecommendations2026'));
assert.ok(nepalGenZTransition?.sourceIds.includes('nepalElectionCommission2026'));
...
assert.equal(filterGeographyCases({ country: 'tunisia' }).find(({ id }) => id === 'tunisian-post-2022-presidential-constitutional-order')?.endYear, 2026);
...
assert.ok(filterGeographyCases({ country: 'senegal' }).some(({ id }) => id === 'senegalese-post-2024-governing-majority-and-constitutional-economic-contestation'));
