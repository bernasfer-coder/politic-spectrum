...
assert.equal(filterGeographyCases({ country: 'map-454' })[0].id, 'malawian-2025-general-election-and-presidential-alternation');
assert.ok(filterGeographyCases({ q: 'Malawi' }).some(({ id }) => id === 'malawian-2025-general-election-and-presidential-alternation'));
assert.equal(filterGeographyCases({ country: 'map-450' })[0].id, 'malagasy-2025-protest-military-takeover-and-refoundation-transition');
assert.ok(filterGeographyCases({ q: 'Madagascar' }).some(({ id }) => id === 'malagasy-2025-protest-military-takeover-and-refoundation-transition'));
assert.equal(filterGeographyCases({ country: 'map-024' })[0].id, 'angolan-2017-transition-2025-electoral-and-protest-contestation');
assert.ok(filterGeographyCases({ q: 'Angola' }).some(({ id }) => id === 'angolan-2017-transition-2025-electoral-and-protest-contestation'));
assert.ok(!('scores' in GEOGRAPHY_LABELS.find(({ id }) => id === 'malagasy-2025-protest-military-takeover-and-refoundation-transition')));
assert.ok(!('scores' in GEOGRAPHY_LABELS.find(({ id }) => id === 'angolan-2017-transition-2025-electoral-and-protest-contestation')));
assert.ok(!('scores' in GEOGRAPHY_LABELS.find(({ id }) => id === 'malawian-2025-general-election-and-presidential-alternation')));
...
assert.ok(filterGeographyCases({ country: 'mali' }).some(({ id }) => id === 'malian-2025-party-dissolution-and-transition-charter-revision'));
...
assert.ok(filterGeographyCases({ country: 'iraq' }).some(({ id }) => id === 'iraqi-2025-electoral-and-government-formation'));
