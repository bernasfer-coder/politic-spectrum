...
assert.equal(GEOGRAPHY_CASES.length, 27);
assert.equal(GEOGRAPHY_LABELS.length, 25);
...
assert.equal(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).length, 5, 'the atlas should expose all dated post-2000 implemented cases');
assert.deepEqual(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).map(({ id }) => id).sort(), ['antarctic-treaty-system', 'bhutanese-democratic-constitutional-transition', 'ghanaian-fourth-republic-constitutional-transition', 'saudi-basic-law-shura-order', 'tongan-constitutional-reform-1875-2010']);
...
console.log('Geography tests passed: 27 dated cases, 25 unscored labels, citations, boundaries, filters and share URLs.');
