...
assert.equal(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).length, 136);
...
assert.equal(filterGeographyCases({ country: 'map-340' }).length, 1);
assert.equal(filterGeographyCases({ country: 'map-340' })[0].id, 'honduran-2025-general-election-and-presidential-transition');
assert.match(filterGeographyCases({ country: 'map-340' })[0].limitation, /event-specific book-length scholarship was not located/);
...
assert.equal(filterGeographyCases({ country: 'nepal' }).find(({ id }) => id === 'nepali-2025-protest-transition-and-2026-election')?.id, 'nepali-2025-protest-transition-and-2026-election');
