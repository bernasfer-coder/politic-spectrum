...
assert.equal(GEOGRAPHY_CASES.length, 143);
assert.equal(GEOGRAPHY_LABELS.length, 141);
...
assert.equal(filterGeographyCases({ continent: 'Oceania' }).length, 10);
...
assert.equal(filterGeographyCases({ period: '2000-onward', relationship: 'implemented' }).length, 121, 'the atlas should expose all dated post-2000 implemented cases');
...
console.log('Geography tests passed: 143 dated cases, 141 unscored labels, citations, boundaries, filters and share URLs.');
