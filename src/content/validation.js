...
const CORE_DIMENSION_IDS = ['economic', 'social', 'authority', 'identity', 'foreign'];
...
const CANDIDATE_FACETS = [
  ...
  { id: 'religion', label: 'Religion / secularism', priority: 'high', treatment: 'separate facet or module' },
...
const VALIDATION_CRITERIA = [
  ...
  { id: 'incremental-value', label: 'Incremental value beyond 5D', mandatory: true },
...
const M4_MODELS = CORE_DIMENSION_IDS.map((removedDimensionId) => ({
  id: `M4-without-${removedDimensionId}`,
  family: 'M4',
  axisIds: CORE_DIMENSION_IDS.filter((dimensionId) => dimensionId !== removedDimensionId),
  facetIds: [],
  purpose: `Leave out ${removedDimensionId} to test whether that core dimension adds distinct information.`,
  status: 'planned; empirical comparison not run',
}));
...
const M5_MODEL = {
  ...
  facetIds: [],
  purpose: 'Stable didactic product baseline; existing scores and cache schema must remain compatible.',
  status: 'current product baseline',
};

const M6_MODELS = CANDIDATE_FACETS.map((facet) => ({
  id: `M6-with-${facet.id}`,
  family: 'M6',
  axisIds: [...CORE_DIMENSION_IDS, facet.id],
  facetIds: [facet.id],
  purpose: `Test whether ${facet.label} adds information beyond the 5D core without unacceptable overload.`,
  status: 'planned; blocked until a separate item bank and participant data exist',
}));

const M7_MODELS = CANDIDATE_FACETS.flatMap((left, leftIndex) => CANDIDATE_FACETS.slice(leftIndex + 1).map((right) => ({
  id: `M7-with-${left.id}-${right.id}`,
  family: 'M7',
  axisIds: [...CORE_DIMENSION_IDS, left.id, right.id],
  facetIds: [left.id, right.id],
  purpose: `Preregistered pair comparison for ${left.label} plus ${right.label}.`,
...
const VALIDATION_MODEL_DEFINITIONS = [
  ...
  {
    id: 'M5-plus-facets',
    family: 'M5-plus-facets',
    axisIds: [...CORE_DIMENSION_IDS],
    facetIds: CANDIDATE_FACETS.map(({ id }) => id),
    purpose: 'Keep five public coordinates while testing whether facets improve explanation and filtering.',
...
const VALIDATION_STATUS = {
  ...
  coreModel: M5_MODEL.id,
  empiricalStatus: 'not-run',
  evidenceBoundary: 'Synthetic or randomly generated answers may exercise code paths but cannot establish reliability, validity, comprehension, invariance, or incremental value.',
  nextEvidence: 'Run preregistered cognitive interviews and a consented, privacy-safe participant pilot before changing the public 5D model.',
};
