...
test('Bolivia 2025–2026 case exposes its dated boundaries and linked evidence', async ({ page }) => {
  await page.goto('/#geography?case=bolivian-2025-electoral-transition-and-stabilization');
  const card = page.locator('.geo-card');
  await expect(card).toHaveCount(1);
  await expect(card).toContainText('2025–2026 · general election, constitutional transfer, post-MAS-dominance realignment');
  await expect(card.locator('.geo-claim')).toContainText('54.96%');
  await expect(card.locator('.geo-limit')).toContainText('not a score or representative description of Bolivia');
  await card.locator('.geo-place-detail summary').click();
  await expect(card.locator('.geo-place-detail')).toContainText('two distinct bounded periods');
  await expect(card.locator('.geo-place-detail')).toContainText('No 2026 outcome study or independent book-length account');
  await card.locator('.geo-evidence summary').click();
  await expect(card.getByRole('link', { name: /Fundación UNIR Bolivia.*second part/i })).toHaveAttribute('href', 'https://unirbolivia.org/sites/default/files/2025-12/del-desencuentro-a-la-crisis-2.pdf');
});
