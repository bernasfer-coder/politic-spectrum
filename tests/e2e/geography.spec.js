...
test('Greece 2015 case distinguishes the referendum question, official return and later negotiations', async ({ page }) => {
  await page.goto('/#geography?case=greece-2015-referendum-and-negotiations');
  const card = page.locator('.geo-card');
  await expect(card).toHaveCount(1);
  await expect(card).toContainText('3,558,864 (61.31%) “No”');
  await expect(card).toContainText('not a direct vote to leave or remain in the euro');
  await expect(card).toContainText('one common reason for voting');
  await expect(card).toContainText('chapter was not fully consulted');
