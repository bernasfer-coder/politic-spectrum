...
test('Philippines impeachment case separates the 2025 ruling from the pending 2026 Senate trial', async ({ page }) => {
  await page.goto('/#geography?case=philippines-duterte-impeachment-constitutional-process-2025-26');
...
test('Morocco 2026 election snapshot distinguishes committee-proclaimed returns, turnout discrepancy and CNDH limits', async ({ page }) => {
  await page.goto('/#geography?case=moroccan-2026-legislative-election-and-preliminary-observation');
  const card = page.locator('.geo-card');
  await expect(card).toHaveCount(1);
  await expect(card).toContainText('23–26 September 2026');
  ...
  await expect(card).toContainText('“definitive” constituency results');
  await expect(card).toContainText('not a Constitutional Court judgment or disposition of any petition');
  await expect(card).toContainText('portal still displayed its result/validation workflow as in progress');
  ...
  await expect(card).toContainText('30 days after announcement of the poll result');
  await expect(card).toContainText('declared winners remain in office until the Court annuls their election');
  await expect(card).toContainText('the 30-day appeal period has not been calculated');
  await card.locator('.geo-evidence summary').click();
  await expect(card.locator('a[href="#bibliography/research-moroccoInteriorLegislativeResults2026"]')).toBeVisible();
  await expect(card.locator('a[href="#bibliography/research-moroccoInteriorDefinitiveCommitteeResults2026"]')).toBeVisible();
  ...
  await expect(card.locator('a[href="#bibliography/research-moroccoConstitutionalCourtElectionDisputesLaw2026"]')).toBeVisible();
  await expect(card.locator('a[href="#bibliography/research-moroccoOrganicLawHouseRepresentatives2026"]')).toBeVisible();
...
test('Djibouti case distinguishes provisional observer denominators from the Constitutional Council final result', async ({ page }) => {
  await page.goto('/#geography?case=djibouti-2025-amendment-and-2026-presidential-election');
  const card = page.locator('.geo-card');
  await expect(card).toHaveCount(1);
  await expect(card).toContainText('204,874 votes, 97.01%');
  await expect(card).toContainText('261,857 registered');
  await expect(card).toContainText('256,467 registered');
...
test('São Tomé and Príncipe election remains a pre-poll case and is reachable without a tiny-island map polygon', async ({ page }) => {
  ...
  await expect(card).toContainText('Acórdão n.º 50/2026');
  await expect(card).toContainText('granted named coalition candidates’ request');
  await expect(card).toContainText('sent to the Public Prosecutor');
  await expect(card).toContainText('petition and complaint');
  await expect(card).toContainText('later filing’s disposition was not located');
  await expect(card).toContainText('scan has no extractable text layer');
  ...
  await expect(card).toContainText('24:00 on the antevéspera of election day');
  await expect(card).toContainText('does not establish that all campaign actors complied');
...
test('Bosnia 2026 pre-election snapshot distinguishes administrative self-report from implementation', async ({ page }) => {
  ...
  await expect(card).toContainText('An English-language N1 page carrying FENA reporting separately says the CEC imposed fines in several cases concerning premature campaigning and minors’ participation in political activity');
  await expect(card).toContainText('the underlying CEC decisions were not directly retrieved');
  ...
  await expect(card.locator('a[href*="bihCec61stSessionN1FenaReport20260924"]')).toBeVisible();
});
...
test('South Sudan transition distinguishes legal instruments, caretaker reporting and disputed inclusion', async ({ page }) => {
  ...
  await expect(card).toContainText('voter registration had not started');
  await expect(card).toContainText('not evidence of subsequent status or an assessment of national election readiness');
...
test('Eritrea case distinguishes ratified constitutional design from later implementation', async ({ page }) => {
  await page.goto('/#geography?case=eritrea-post-liberation-constitutional-transition');
  const card = page.locator('.geo-card');
  await expect(card).toHaveCount(1);
  await expect(card).toContainText('ratified Eritrea’s Constitution on 23 May 1997');
  await expect(card).toContainText('the Constitution was never implemented');
