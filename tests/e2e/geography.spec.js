...
test('geographic filters, timeline, refresh and history preserve the selected view', async ({ page }) => {
  ...
  await page.getByRole('combobox', { name: /Country/ }).selectOption('map-332');
  await expect(page.locator('.geo-card')).toHaveCount(2);
  await expect(page.locator('.geo-card').nth(1)).toContainText('Haitian post-Moïse transition and electoral horizon');
