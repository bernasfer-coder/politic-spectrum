...
test('Peru’s 2026 election is shown as a bounded, sourced case', async ({ page }) => {
  await page.goto('/#geography?country=map-604');
  const cards = page.locator('.geo-card');
  await expect(cards).toHaveCount(3);
  const card = cards.filter({ hasText: 'Peruvian 2026 general election, bicameral return and presidential runoff' });
  await expect(card).toContainText('April–July 2026');
  await expect(card).toContainText('no six-axis placement or score is assigned');
  await card.locator('.geo-evidence summary').click();
  await expect(card.getByRole('link', { name: 'Bibliography & rights record →' })).toHaveCount(11);
});
