...
describe('map geometry and filter contracts', () => {
  it('keeps all map IDs unique, finite and compatible with share URLs', () => {
    ...
    expect(MAP_COUNTRIES_BY_ID['map-752'].name).toBe('Sweden');
    expect(MAP_COUNTRIES_BY_ID['map-008'].name).toBe('Albania');
    expect(MAP_COUNTRIES_BY_ID['map-498'].name).toBe('Moldova');
...
describe('interactive atlas map', () => {
  ...
  it('supports roving keyboard focus, Enter and Space without creating a tab stop per country', async () => {
    ...
    expect(cards()).toHaveLength(4); // Nasser, post-Nasser, the 2025–2026 election and an Egyptian Ottoman-network node.
