...
describe('geographic atlas', () => {
  ...
  it('keeps the 2025 Moldova election bounded to its institutions and evidence', async () => {
    const user = userEvent.setup();
    renderAtlas();
    await user.selectOptions(screen.getByRole('combobox', { name: /Country/ }), 'map-498');
    expect(cards()).toHaveLength(1);
    expect(cards()[0]).toHaveTextContent('Moldovan 2025 parliamentary election and government transition');
    expect(cards()[0]).toHaveTextContent('PAS 55 seats');
    expect(cards()[0]).toHaveTextContent(/No synthetic six-axis score is warranted/i);
    expect(cards()[0]).toHaveTextContent('not a score or ideological profile for Moldovan citizens');
  });
