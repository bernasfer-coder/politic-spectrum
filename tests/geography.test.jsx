...
describe('geographic atlas', () => {
  ...
  it('shows the 2023–2025 Sudan war as a separate bounded case, not a current-country score', async () => {
    const user = userEvent.setup();
    renderAtlas();
    await user.selectOptions(screen.getByRole('combobox', { name: /Country/ }), 'sudan');
    expect(cards()).toHaveLength(2);
    expect(cards()[0]).toHaveTextContent('1989–2021');
    expect(cards()[1]).toHaveTextContent('2023–2025');
    expect(cards()[1]).toHaveTextContent('not a current 2026 status claim');
    expect(cards()[1]).toHaveTextContent('not a criminal court’s judgment');
  });
