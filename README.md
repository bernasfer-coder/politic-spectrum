...
Its interactive, self-hosted map lets visitors click countries/territories, explore city/region markers, and select cross-border networks. It supports zoom, mouse panning, keyboard selection, mobile controls and shareable geographic filters. Grey areas can be selected too and display honest research gaps. Natural Earth 4.1.0 geometry (via world-atlas 2.0.2) is a simplified reference, not a current or historical control map; data/software notices are available from the map. No external map tiles, tracking, geolocation or per-click API calls are used.
...
The same library now contains a Research Atlas. It separates primary works and legal texts from documented people, represents positions as ranges with claim-level locations, records original language and translation limits, shows a dimension/band coverage matrix, preserves cautious relationships such as `close_to`, `distinct_from`, `influenced`, and `opposed`, and keeps a prioritized bibliography backlog visible. The expanded inventory contains 65 works and legal texts plus 24 documented people spanning ancient Egypt and Mesopotamia, Greek and Roman constitutional thought, Christian and medieval legal-political texts, classical Chinese schools, South Asian and Buddhist sources, Islamic and Persian political thought, Jewish law, Southeast Asian anti-colonial thought, Mesoamerican records, Pacific constitutional history, West African and Indigenous constitutional memory, and the early-modern constitutional, liberal, feminist, realist, and socialist canon. The atlas also shows 12 recurring political forms with linked source anchors. The language filter identifies English translations rather than silently treating them as originals. Empty or thin cells are intentional research gaps, not fabricated coverage. The scope and source boundaries are documented in [`CLASSICAL_POLITICAL_CANON.md`](CLASSICAL_POLITICAL_CANON.md).
...
Content is intentionally framed as approximate and educational: a person, country, state, or city cannot be reduced to six numbers. Italian/interwar fascism and National Socialism/Nazism have separate reference cards, documented examples, and encyclopedia pages, with their historical relationship explained rather than treated as interchangeable. Both are included only for critical historical analysis. Each selected reference card links directly to its full encyclopedia entry.
